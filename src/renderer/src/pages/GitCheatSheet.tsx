...
const GIT_COMMANDS: GitCategory[] = [
  {
    ...
    commands: [
      ...
      { command: 'git clone <url>', description: '克隆远程仓库', example: 'git clone https://github.com/user/repo.git' }
    ]
  },
  {
    id: 'branch',
    ...
    commands: [
      ...
      { command: 'git branch <name>', description: '创建新分支', example: 'git branch feature/login' },
      { command: 'git branch -d <name>', description: '删除分支', example: 'git branch -d feature/login' },
      { command: 'git checkout <branch>', description: '切换到指定分支', example: 'git checkout main' },
      { command: 'git checkout -b <name>', description: '创建并切换到新分支', example: 'git checkout -b feature/login' },
      { command: 'git switch <branch>', description: '切换分支（推荐）', example: 'git switch main' },
      { command: 'git switch -c <name>', description: '创建并切换新分支（推荐）', example: 'git switch -c feature/login' },
      { command: 'git merge <branch>', description: '将指定分支合并到当前分支', example: 'git merge feature/login' },
      { command: 'git rebase <branch>', description: '变基当前分支到指定分支', example: 'git rebase main' }
    ]
  },
  {
    id: 'commit',
    ...
    commands: [
      ...
      { command: 'git commit -m <msg>', description: '提交暂存区的更改', example: 'git commit -m "feat: add login"' },
      { command: 'git commit --amend', description: '修改最近一次提交', example: 'git commit --amend -m "fix: typo"' },
      { command: 'git reset HEAD~1', description: '撤销最近一次提交（保留更改）', example: 'git reset --soft HEAD~1' },
      { command: 'git reset --hard HEAD~1', description: '撤销最近一次提交（丢弃更改）', example: 'git reset --hard HEAD~1' },
      { command: 'git revert <commit>', description: '创建一个新提交来撤销指定提交', example: 'git revert a1b2c3d' }
    ]
  },
  {
    id: 'remote',
    ...
    commands: [
      ...
      { command: 'git remote add <name> <url>', description: '添加远程仓库', example: 'git remote add origin https://github.com/user/repo.git' },
      ...
      { command: 'git pull --rebase', description: '拉取并变基', example: 'git pull --rebase origin main' },
      { command: 'git push', description: '推送到远程仓库', example: 'git push origin main' },
      { command: 'git push -u origin <branch>', description: '推送并设置上游分支', example: 'git push -u origin feature/login' },
      { command: 'git push origin --delete <branch>', description: '删除远程分支', example: 'git push origin --delete feature/login' }
    ]
  },
  {
    id: 'stash',
    ...
    commands: [
      ...
      { command: 'git stash push -m <msg>', description: '暂存并添加描述', example: 'git stash push -m "wip: login page"' },
      ...
      { command: 'git stash apply', description: '恢复暂存（不删除）', example: 'git stash apply stash@{0}' },
      { command: 'git stash drop', description: '删除最近一次暂存', example: 'git stash drop stash@{0}' },
  ...
  },
  {
    ...
    commands: [
      ...
      { command: 'git tag -a <name> -m <msg>', description: '创建附注标签', example: 'git tag -a v1.0.0 -m "Release v1.0.0"' },
      { command: 'git push origin <tag>', description: '推送标签到远程', example: 'git push origin v1.0.0' },
      { command: 'git push origin --tags', description: '推送所有标签', example: 'git push origin --tags' },
  ...
  },
  {
    ...
    commands: [
      ...
      { command: 'git log --oneline', description: '简洁模式查看历史', example: 'git log --oneline' },
      { command: 'git log --graph', description: '图形化查看历史', example: 'git log --graph --oneline --all' },
      ...
      { command: 'git blame <file>', description: '查看文件每一行的最后修改', example: 'git blame index.ts' },
      { command: 'git diff', description: '查看未暂存的更改', example: 'git diff' },
      { command: 'git diff --staged', description: '查看已暂存的更改', example: 'git diff --staged' },
      { command: 'git diff <branch1> <branch2>', description: '比较两个分支', example: 'git diff main feature/login' }
    ]
  },
  {
    id: 'cherry-pick',
    ...
    icon: '🍒',
    commands: [
      { command: 'git cherry-pick <commit>', description: '将指定提交复制到当前分支', example: 'git cherry-pick a1b2c3d' },
      { command: 'git cherry-pick <commit1> <commit2>', description: '复制多个提交', example: 'git cherry-pick a1b2c3d e4f5g6h' }
    ]
  }
]
